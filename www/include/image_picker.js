var ImagePicker = new Class.create();

ImagePicker.prototype = {

    initialize: function(divid, editorid, parentid, headid, args) {
        this.images = {};
        this.placeholders = {};
        ...
        $('imageEditor_rescale').observe('keypress', this.fixSubmit.bindAsEventListener(this));
        $('imageEditor_altText').observe('keypress', this.fixSubmit.bindAsEventListener(this));
        $('imageEditor_caption').observe('keypress', this.fixSubmit.bindAsEventListener(this));
        $('imageEditor_photoCredit').observe('keypress', this.fixSubmit.bindAsEventListener(this));
    },
    ...
    editImageOld: function(id) {
        window.open('/imagedb/upload_image_form.html?category=profiles&parentid=' + this.parentid + '&indexid=' + id , 'blah', 'toolbar=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=1,width=300,height=300');
    },
    ...
    addImage: function (id, imgtag, tsizey, iid, info) {
        if ($defined(this.images[id]))
        {
            ...
            Event.observe(img_div, 'click', this.insertImageTag.bindAsEventListener(this, this.editorid, '{{image|' + id + '}}'));
    ...
    showCustomPane: function(pane) {
    	this.editing = null;
    	this.editor.hide();
    	this.container.hide();
    	
    	if (this.customPane) {
    		this.customPane.remove();
    	}
    	
    	this.customPane = pane;
    	this.container.insert({ after: this.customPane });
    },
    
    hideCustomPane: function() {
    	if (!this.customPane)
    		return;
    	
    	this.customPane.remove();
    	this.customPane = null;
