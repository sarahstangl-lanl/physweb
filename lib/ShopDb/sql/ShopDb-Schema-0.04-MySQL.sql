...
-- Created on Sun Jan 22 15:00:07 2012
-- 
...
CREATE TABLE `shopdb`.`attachments` (
  ...
  `uploader_id` integer NOT NULL DEFAULT 21,
  INDEX `shopdb.attachments_idx_uploader_id` (`uploader_id`),
  PRIMARY KEY (`attachment_id`),
  CONSTRAINT `shopdb.attachments_fk_uploader_id` FOREIGN KEY (`uploader_id`) REFERENCES `shopdb`.`customers` (`customer_id`)
) ENGINE=InnoDB;
...
CREATE TABLE `shopdb`.`job_comments` (
  ...
  INDEX `shopdb.job_comments_idx_creator_id` (`creator_id`),
  INDEX `shopdb.job_comments_idx_job_id` (`job_id`),
  PRIMARY KEY (`job_comment_id`),
  CONSTRAINT `shopdb.job_comments_fk_creator_id` FOREIGN KEY (`creator_id`) REFERENCES `shopdb`.`customers` (`customer_id`),
