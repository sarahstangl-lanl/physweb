...
$VERSION  = '0.20';
$ATTRIBUTION = "DBD::AnyData $DBD::AnyData::VERSION by Jeff Zucker, Jens Rehsack, Lucio Iacolettig";
...
sub validate_STORE_attr {
     ...
     if ($attrib eq "f_dir") {
         ...
         File::Spec->file_name_is_absolute($value) or $value = Cwd::abs_path($value);
     }
...
sub ad_get_catalog {
     ...
     if($tname) {
         return($self->{ad_tables}->{$tname}) if($self->{ad_tables}->{$tname});
...
sub ad_export {
     ...
     $data = $dbh->selectall_arrayref("SELECT * FROM $table_name") if($format =~ /XML|HTMLtable/);
...
sub ad_convert {
     ...
     if ($format eq 'DBI') {
         ...
         die('table_name required to convert DBI') unless($_[1] and $_[1]->{table_name});
     ...
     if($old_records) {
         ...
         while(my $row = $sth->fetchrow_hashref) {
             ...
             if($flags->{lookup_key}) {
                 ...
                 if($dup) {
                     $dbh->do("DELETE FROM temp__ WHERE $lookup = $val") or die(DBI->errstr);
                 }
             }
...
sub ad_catalog {
     ...
     for my $one_table (@{$table_info}) {
         ...
         die('ERROR: ad_catalog requires a table name!') unless($flags->{table_name});
...
sub open_data {
     ...
     my $ad     = $catalog->{ad} || AnyData::adTable($format, $file, $createMode, $lockMode, $catalog, $tname);
...
table and print out "Just Another Perl Hacker":
     ...
     print $dbh->selectcol_arrayref(qq{SELECT phrase FROM test WHERE id = 2})->[0];
